...
pcapPath =f +'/pcaps/captured_packets.pcap'
filename=pcapPath.replace(".pcap","_FE.csv")
print("Đang lắng nghe gói tin...")
packets = sniff(iface="Wi-Fi", count=100, store=1) 
...
with open(filename, mode='w', newline='') as file:
    ...
    for packet in packets:
        ...
        # Ghi vào file CSV
        writer.writerow(row)
    # Lắng nghe gói tin từ giao diện Wi-Fi (thay 'Wi-Fi' bằng tên giao diện mạng của bạn)
...
df = pd.read_csv('./SW/captured_packets_SW.csv',usecols=cols)#,header=None )
df=df.fillna(0)
df=df[cols]
...
print(f"Dữ liệu đã được ghi vào folder 'SW")
